...
def search_companies_positive_income(api_client, symbols, year, net_csv_path,
                                     operating_csv_path):
    now = datetime.datetime.today()
    from_date = now - datetime.timedelta(days=(year * 365))
    ...
    for c_symbols in divide_chunks(symbols, 100):
        ...
        histories = api_client.get_income_statement_history(
            identifiers=c_symbols)

        for history in histories:
            if history.has_positive_net_income_from(
                    from_date=from_date, type="net"):
                output_net.append((history.symbol, history.name))
            if history.has_positive_net_income_from(from_date=from_date,
                                                    type="operating"):
